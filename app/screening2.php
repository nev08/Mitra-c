...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare default values for q1 to q10
    $q_values = array('no', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 'no');
    ...
    $check_sql = "SELECT * FROM screening2 WHERE patient_id = '$patient_id' AND `date` = '$today_date'";
    $check_result = mysqli_query($conn, $check_sql);

    if(mysqli_num_rows($check_result) == 0) {
        // Insert default values into the table for the specific patient
        $sql = "INSERT INTO screening2 (patient_id, q1, q2, q3, q4, q5, q6, q7, q8, q9, q10, `date`) 
